...
// INFO:
// Only the URL differs between these, but the implementation could differ significantly
// if we decide to support windows, so leaving the implementation duplicated for now
...
impl InstallStrategy for MacSiliconClient {
    fn install(&self, version_num: &str) -> Result<(), Error> {
        ...
        let url = format!(
            "https://nodejs.org/dist/v{version_num}/node-v{version_num}-darwin-arm64.tar.xz"
        );
        let response = ureq::get(&url).call();
